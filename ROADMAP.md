...
## ✅ وضعیت فعلی (Done)
- **صفحهٔ تعاملی (browser):** ساخت کلید X25519 کاملاً در مرورگر (مزیت اختصاصی)، چند SNI/پورت خودکار، حالت‌های سریع/WARP/هردو/بدون‌SNI، نصب ایزولهٔ مقاوم به قطع SSH.
...
- **هسته:** VLESS Reality+Vision، WARP outbound، Shadowsocks، Multi-user (UUID/سهمیه/انقضا)، CLI مدیریتی، Watchdog، BBR، Kv2m دسکتاپ (Win/مک/لینوکس Tkinter + CLI/Termux).

---

## 🔧 فاز ۰ — رفع باگ‌های بحرانی (قبل از هر فاز جدید)
> این‌ها در v2.4 نبودند ولی برای پایداری حیاتی‌اند — اول انجام شوند.

- 🔴 **۰.۱ تداخل پورت API (کرش‌لوپ روی سرورِ دارای پنل دیگر):** پورت `10085` با 3x-ui/Marzban تداخل می‌کند → API پورتِ تصادفی/قابل‌تنظیم + گاردِ ضدتداخل + تشخیص در `status`.
- 🔴 **۰.۲ پایدارسازی WARP:** گذر از MASQUE به WireGuard، ثبت‌نام مطمئن‌تر، **fallback خودکار** (اگر WARP وصل نشد → کانفیگ‌های warp موقتاً direct) + اطلاع در status.
- 🔴 **۰.۳ خودتشخیصیِ `status`:** تشخیص خودکارِ کرش‌لوپ/WARP مرده/تداخل پورت/SS غیرفعال و چاپِ **همان یک دستورِ رفع**.
- 🟠 **۰.۴ بهینه‌سازی سرعت (BBR):** مرحلهٔ امنِ tune در نصب (`bbr`+`fq`+بافر+`fastopen`+`mtu_probing`+`conntrack`، idempotent با بکاپ/rollback). مرجع: تا ۸× سریع‌تر.
- 🟠 **۰.۵ نصب مجدد امن:** نصب دوباره نباید کاربران قبلی را پاک کند؛ ادغام/هشدار + راهنمایی واضح.
- 🟠 **۰.۶ رفع/بازطراحی Shadowsocks:** تأیید زندهٔ علتِ پینگ‌ندادن و بازطراحیِ تجربهٔ SS برای پایداری.
...
*نسخه رودمپ: 2.4.0 | آخرین بروزرسانی: تیر ۱۴۰۵*
