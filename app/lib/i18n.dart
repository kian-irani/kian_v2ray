...
class Strings {
  ...
  static const Map<String, List<String>> _t = {
    ...
    'setup.tls': ['از دامنه استفاده می‌کنی؟', 'Use a domain?'],
    'setup.tls.d': ['اگر دامنه داری، می‌توانی پروتکل‌ها را خودت انتخاب کنی (TLS هم اضافه می‌شود). اگر نه، Reality + Shadowsocks + Hysteria2 + TUIC خودکار فعال می‌شوند.', 'With a domain you pick the protocols yourself (TLS too). Without one, Reality + Shadowsocks + Hysteria2 + TUIC are auto-enabled.'],
    'setup.pickproto': ['پروتکل‌هایی که می‌خواهی فعال شوند را تیک بزن:', 'Tick the protocols you want enabled:'],
    'setup.nodomainwarn': ['این‌ها دامنه نمی‌خواهند و IP سرورت در کانفیگ دیده می‌شود:', 'These need no domain — your server IP shows in the config:'],
    'setup.tlsdomain': ['دامنه (رکورد A به IP سرور)', 'Domain (A record to server IP)'],
    'setup.autoproto': ['پروتکل‌های جانبی خودکار فعال می‌شوند', 'Companion protocols are auto-enabled'],
    'setup.autoproto.d': ['Shadowsocks، Hysteria2 و TUIC v5 بدون نیاز به تنظیم برای همهٔ کاربران فعال می‌شوند. یک سرویسِ sing-box کنارِ Xray نصب می‌شود — هر کاربر برای هر پروتکل لینک جداگانه می‌گیرد.', 'Shadowsocks, Hysteria2 and TUIC v5 are enabled for all users automatically. A sing-box service is installed next to Xray — each user gets a separate link per protocol.'],
    ...
    'setup.update': ['به‌روزرسانیِ سرور', 'Update server'],
    'setup.update.d': ['سرورِ از-قبل-نصب‌شده را به‌روز می‌کند: Xray + اسکریپت‌ها + پنل + پروتکل‌های جدید (مثلِ AnyTLS) + همگام‌سازیِ همهٔ لینک‌های Subscription — بدونِ نصبِ دوباره، کاربران حفظ می‌شوند.', 'Updates an already-installed server: Xray + scripts + panel + new protocols (e.g. AnyTLS) + resyncs every subscription — no reinstall, users kept.'],
    'setup.update.btn': ['به‌روزرسانیِ سرور (همان IP و SSH بالا)', 'Update server (uses the IP & SSH above)'],
    'setup.update.need': ['اول IP سرور و رمزِ SSH را بالا وارد کن.', 'Enter the server IP and SSH password above first.'],
    'setup.update.connecting': ['اتصالِ SSH به', 'SSH to'],
    'setup.update.running': ['متصل شد. در حالِ به‌روزرسانی (۲–۵ دقیقه)…', 'Connected. Updating (2–5 min)…'],
    'setup.update.done': ['سرور به‌روز شد. قابلیت‌های جدید اضافه و لینک‌ها سینک شد.', 'Server updated. New features added and links resynced.'],
    'setup.update.fail': ['به‌روزرسانی با خطا تمام شد', 'Update finished with an error'],
    'setup.uninstall': ['حذف نصب از سرور', 'Uninstall from server'],
    'setup.uninstall.d': ['Xray، پنل، و همه تنظیمات را از سرور پاک می‌کند. غیرقابل‌بازگشت.', 'Removes Xray, panel, and all configs from the server. Irreversible.'],
    'setup.uninstall.btn': ['حذف کامل از سرور (همان IP و SSH بالا)', 'Fully uninstall from server (uses IP & SSH above)'],
    'setup.uninstall.confirm': ['مطمئنی؟ Xray، پنل، و همه تنظیمات از سرور پاک می‌شوند.', 'Sure? Xray, panel, and all configs will be removed from the server.'],
