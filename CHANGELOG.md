...
## [v4.2.0] — 2026-06-25  (پنلِ اجباری + پروتکل‌های خودکار + حذف از سرور)

### افزوده شد
- **حذف از سرور در اپ اندروید** (`setup_screen.dart`): دکمهٔ «حذف کامل از سرور» با دیالوگ تأیید — Xray، پنل، و همهٔ تنظیمات را از VPS پاک می‌کند. از طریق SSH (همان IP/رمز بالا).
- **پروتکل‌های خودکار در اپ اندروید** (`setup_screen.dart`): وقتی TLS/دامنه تنظیم نشده، Shadowsocks + Hysteria2 + TUIC v5 به‌طور خودکار فعال می‌شوند (بدون نیاز به تیک‌زدن) — یک کارتِ سبز توضیح می‌دهد. وقتی TLS فعال است، انتخاب دستی برمی‌گردد.
- **جدول دانلود در Release** (`.github/workflows/release.yml`): release body حالا جدولی از همهٔ artifact ها با نام پلتفرم دارد.
