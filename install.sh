...
# ── Server console output: ENGLISH ONLY ────────────────────────────────────
# The install console is always English so operators of ANY language can follow
# it without confusion. The app/page/panel UIs stay localized and show RESULTS
# (links, status) in the user's own language — the server console does not.
# _tr() maps the Persian milestone/error strings → English (zero call-site
# changes); xlt() picks the English side for interpolated lines.
KIAN_LANG=en
_ENV_LANG="en"
declare -A _EN=(
  ...
  ["پورت‌ها تغییر کردند؛ links.txt و Subscription از config بازسازی شد. لینک صفحه ممکن است قدیمی باشد — از «kian-v2ray sub <name>» لینک به‌روز را بگیر."]="Ports changed; links.txt and the subscription were rebuilt from config. The page link may be stale — get a fresh one with 'kian-v2ray sub <name>'."
)
...
cmd_status(){
  ...
  printf "\n${C_B}━━━ KIAN V2Ray — Status ━━━${C_N}\n\n"
  local issues=()   # هر مورد: "متن مشکل|||دستور رفع"
  if [ -f "$LOCK_FILE" ] && kill -0 "$(cat "$LOCK_FILE" 2>/dev/null)" 2>/dev/null; then
    warn "Install still running — live log:  bash $0 logs"
  fi
  ...
  if command -v docker >/dev/null 2>&1; then
    ...
    case "$cstatus" in
      running)
        xrunning=1
        say "Xray is running (container: $CONTAINER)"
  ...
  if [ -f "$cfg" ] && command -v jq >/dev/null 2>&1 && command -v ss >/dev/null 2>&1; then
    ...
    if [ -n "$down" ]; then
      err "Port(s) that should be open are not listening:$down"
      issues+=("پورت شنونده نیست:$down|||docker restart $CONTAINER ; docker logs --tail 40 $CONTAINER")
    else
      printf "\n${C_B}Service ports (all listening):${C_N}\n"
    ...
    if [ -n "${ssport:-}" ]; then
      printf '%s\n' "$listen" | grep -qx "$ssport" && say "Shadowsocks active (port $ssport)" || warn "Shadowsocks down (port $ssport)"
    fi
  fi
  ...
  if [ "${#issues[@]}" -eq 0 ]; then
    ...
    err "${#issues[@]} issue(s) found — fix command for each:"
    local i=1 it
    for it in "${issues[@]}"; do
      printf "  ${C_Y}%d) %s${C_N}\n     ${C_B}Fix:${C_N} %s\n" "$i" "${it%%|||*}" "${it##*|||}"
      i=$(( i + 1 ))
  ...
  printf "\n${C_B}Configs:${C_N}  kian-v2ray configs\n\n"
}
cmd_logs(){ [ -f "$INSTALL_LOG" ] || { err "No logs yet."; exit 1; }; tail -n 60 -f "$INSTALL_LOG"; }
...
if [ -z "${KIAN_DETACHED:-}" ]; then
  if [ -f "$LOCK_FILE" ] && kill -0 "$(cat "$LOCK_FILE" 2>/dev/null)" 2>/dev/null; then
    ...
    inf  "Status:  bash $0 status   |   Logs:  bash $0 logs"
    exit 0
  fi
  cp -f "$0" "$WORK_DIR/install.sh" 2>/dev/null || curl -fsSL "$RAW_BASE/install.sh" -o "$WORK_DIR/install.sh"
  chmod +x "$WORK_DIR/install.sh"
  printf "\n${C_B}🚀 KIAN V2Ray install started${C_N}\n"
  printf "   (You'll see the steps live. If SSH drops, the install keeps running in the background.)\n\n"
  : > "$INSTALL_LOG"
  ...
  if grep -q "done" "$STATE_FILE" 2>/dev/null || grep -q "نصب با موفقیت کامل شد" "$INSTALL_LOG" 2>/dev/null; then
    printf "${C_G}✅ Install complete!${C_N}\n"
    printf "   Configs:   ${C_B}kian-v2ray configs${C_N}\n"
    printf "   Sub link:  ${C_B}kian-v2ray sub <name>${C_N}\n"
    printf "   Status:     ${C_B}kian-v2ray status${C_N}\n"
  else
    printf "${C_Y}⚠️ Install finished but no definitive result. Check status:${C_N}\n"
    printf "   ${C_B}kian-v2ray status${C_N}  or full log: ${C_B}bash $WORK_DIR/install.sh logs${C_N}\n"
  fi
  exit 0
fi
...
TUNE
  ...
  else warn "BBR may not be active on this kernel (cc=$cc) — not a problem for operation"; fi
}
...
ensure_swap(){
  ...
  if [ "${cur_kb:-0}" -gt 0 ]; then inf "swap already active ($((cur_kb/1024)) MB) — skipped"; return 0; fi
  local ram_mb; ram_mb="$(awk '/MemTotal/{print int($2/1024)}' /proc/meminfo 2>/dev/null || echo 0)"
  [ "${ram_mb:-0}" -gt 0 ] && [ "$ram_mb" -le 2048 ] || { inf "enough RAM (${ram_mb} MB) — swap not needed"; return 0; }
  ...
  if [ "${free_mb:-0}" -lt $((size_mb + 1024)) ]; then
    warn "not enough disk for swap (free: ${free_mb}MB) — skipped"; return 0
  fi
  local sf=/swapfile
  inf "creating swap file (${size_mb} MB)..."
  ...
  if ! fallocate -l "${size_mb}M" "$sf" 2>/dev/null; then
    dd if=/dev/zero of="$sf" bs=1M count="$size_mb" status=none 2>/dev/null \
      || { warn "creating swap file failed — skipped"; rm -f "$sf"; return 0; }
  fi
  chmod 600 "$sf"
  mkswap "$sf" >/dev/null 2>&1 || { warn "mkswap failed (likely OpenVZ/LXC) — skipped"; rm -f "$sf"; return 0; }
  swapon "$sf" 2>/dev/null      || { warn "swapon failed (virtualization likely disallows swap) — skipped"; rm -f "$sf"; return 0; }
  ...
  say "swap enabled (${size_mb} MB) — prevents service crashes on low-RAM VPS"
}
if ! done_step swap; then ensure_swap; mark_step swap; else inf "swap already checked — skipped"; fi
...
warp_fallback(){ # $1 = on|off
  # ترافیکِ عمومی همیشه مستقیم است؛ فقط دامنه‌های منتخب (AI) از warp می‌روند.
  # برای fallback به‌جای دستکاریِ rules، خودِ outbound warp را بین socks و
  # freedom سوییچ می‌کنیم — مستقل از ساختارِ routing، پس همیشه درست کار می‌کند.
  ...
  if [ "$1" = "on" ]; then
    ...
    tmp="$(mktemp)"
    jq --argjson port "$WARP_PORT" '(.outbounds[]|select(.tag=="warp"))|={tag:"warp",protocol:"socks",settings:{servers:[{address:"127.0.0.1",port:$port}]}}' "$cfg" > "$tmp" \
      && jq -e . "$tmp" >/dev/null 2>&1 && mv "$tmp" "$cfg" || rm -f "$tmp"
    echo off > "$flag"
    docker restart "$CONTAINER" >/dev/null 2>&1 || true
...
install_warp(){
  if command -v warp-cli >/dev/null 2>&1; then inf "WARP از قبل نصب است"
    ...
    if ! { apt-get update -y -q && apt-get install -y -q cloudflare-warp; }; then
      warn "codename=$codename failed — trying jammy"
  ...
  for proto in WireGuard MASQUE; do
    inf "connecting WARP with protocol $proto..."
    if _warp_try "$proto"; then ok=1; echo "$proto" > "$ETC_DIR/warp_proto.txt"; break; fi
    warn "WARP did not connect with $proto — trying next protocol"
  done
  if [ "$ok" = 1 ]; then
    say "WARP connected (socks5 :$WARP_PORT — protocol $(cat "$ETC_DIR/warp_proto.txt" 2>/dev/null))"
...
# The Xray container runs as --user 0:0 (root, see docker run below), so it can
# write logs without world-writable perms. Keep the dir root-owned and 750 so
# local non-root users can't read or tamper with connection logs (was 777).
...
if _port_busy "$API_PORT"; then
  warn "API port ($API_PORT) is in use — likely another panel (3x-ui/Marzban) is active"
  NEWAPI="$API_PORT"
  ...
  inf "API port changed to $NEWAPI (auto-fix)"
  tmpc="$(mktemp)"
...
if [ -f "$ETC_DIR/users.json" ] && jq -e '.users' "$ETC_DIR/users.json" >/dev/null 2>&1; then
  ...
  warn "previous install found — backup created at $bdir/*_${bts}.*"
  ...
  if [ "${EXTRA_COUNT:-0}" -gt 0 ]; then
    inf "merging $EXTRA_COUNT previous user(s) not present in the new install"
    ...
    warn "to add a new user, prefer the management tab or "kian-v2ray add" rather than reinstalling."
  fi
fi
...
if [ "$(printf '%s' "$SUB_TOKENS" | jq 'length')" -gt 0 ]; then
  ...
  say "Subscription files created ($(printf '%s' "$SUB_TOKENS" | jq 'length') کاربر)"
fi
...
if [ "$COLLISION" = 1 ]; then
  ...
  inf "new reality ports:$NEW_REALITY${SS_PORT_CUR:+ | ss: $SS_NEW}"
fi
...
if [ "$COLLISION" = 1 ] && [ -n "$PBK" ]; then
  if rebuild_links_from_config; then
    ...
    warn "پورت‌ها تغییر کردند؛ links.txt و Subscription از config بازسازی شد. لینک صفحه ممکن است قدیمی باشد — از «kian-v2ray sub <name>» لینک به‌روز را بگیر."
  fi
fi
docker rm -f "$CONTAINER" >/dev/null 2>&1 || true
...
if [ -n "$TLS_DOMAIN_VAL" ] && [ -n "$CADDYFILE_B64" ]; then
  inf "setting up domain configs for: $TLS_DOMAIN_VAL"
  ...
  if [ -n "$RESOLVED_IP" ] && [ "$RESOLVED_IP" != "$SERVER_IP" ]; then
    warn "domain ($TLS_DOMAIN_VAL → $RESOLVED_IP) does not point to this server IP ($SERVER_IP)."
  ...
  if command -v caddy >/dev/null 2>&1; then
    ...
    if systemctl reload caddy >/dev/null 2>&1 || systemctl restart caddy >/dev/null 2>&1; then
      ...
      if systemctl is-active caddy >/dev/null 2>&1; then
        ...
        inf "HTTPS test: curl -I https://$TLS_DOMAIN_VAL"
      else
...
if [ -n "$SUB_OK" ]; then
  say "Subscription service is up (ports: $SUB_OK)"
else
...
if command -v ufw >/dev/null 2>&1 && ufw status 2>/dev/null | grep -qi '^Status: active'; then
  ...
  say "firewall ports opened (SSH: $(echo $SSH_PORTS | tr '\n' ' '))"
else
...
if { [ -n "$EXTRA_PROTOCOLS" ] || [ "${KIAN_EXTRA_PROTOCOLS:-0}" = "1" ]; } \
  ...
  inf "enabling extra protocols (Hysteria2/TUIC on sing-box): ${EXTRA_PROTOCOLS:-env}"
