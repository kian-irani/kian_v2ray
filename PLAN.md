...
- [ ] ۸.۴ **اپ کانفیگ‌ها/سابسکرایب را نشان دهد و مدیریت کند** — نمای جزئیاتِ سرور (URI کامل، کپی، QR)،
  مدیریتِ لینکِ Subscription (نمایش/کپی/رفرش). [بعدی]
- [ ] ۸.۵ **پنل از داخلِ اپ** — سوییچِ «راه‌اندازیِ پنلِ وب» در ویزارد → SSH `kian-v2ray panel enable`
  → نمایشِ URL + user/pass؛ و اطمینان از کارکردِ `manage_screen` روی آن. [بعدی]
- [ ] ۸.۶ **(srv)** تستِ end-to-end روی سرورِ `37.221.79.91`: ویزاردِ موبایل + Hysteria2/TUIC + پنل پشتِ TLS.
